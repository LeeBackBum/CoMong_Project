...
public class NcpUtil {
    ...
    public static String getSentimentDetail(Object sentimentResponse) {
        if (sentimentResponse instanceof JSONObject) {
            JSONObject response = (JSONObject) sentimentResponse;
            JSONObject documentSentiment = (JSONObject) response.get("documentSentiment");
            if (documentSentiment != null) {
                return (String) documentSentiment.get("sentiment"); // 긍정, 부정, 중립 등의 값을 반환
            }
        }
        return "Unknown";
    }
